...
export const useMySqliteStore = defineStore('mySqlite', () => {
  ...
  const getHistoryByDopamineId = async (dopamineId: string) => {
  ...
  //testData check
  const getHistory = async (dopamineId: string) => {
    console.log('4 - get Dopa history by id :' + dopamineId)
    const stmt = `SELECT * FROM history WHERE id_dopamine='${dopamineId}' AND sql_deleted ==0`;
    const values: any[] = [];
    const fetchData = await useQuerySQLite(db, stmt, values);
  ...
  //handle
  const handleDeleteDopamine = async (id: string) => {
    if (db.value) {
      //删除所有的comment
  ...
  const handleDeleteHistoryByDopamineId = async (id: string) => {
    if (db.value) {
      const isConn = await sqliteServ.isConnection(dbNameRef.value, false);
  ...
  const handleAddHistory = async (newHistory: DopaHistory): Promise<string> => {
    if (db.value) {
      const isConn = await sqliteServ.isConnection(dbNameRef.value, false);
  ...
  const handleGetCommentByHistoryId = async (historyId: string): Promise<HistoryComment[]> => {
    if (db.value) {
      const isConn = await sqliteServ.isConnection(dbNameRef.value, false);
  ...
  const handleDeleteCommentByHistoryId = async (id: string) => {
    if (db.value) {
      const isConn = await sqliteServ.isConnection(dbNameRef.value, false);
      const lastId = await storageServ.deleteCommentByHistoryId(id)
    }
  };
  const handleGetHistoryId = async (id: string) => {
    if (db.value) {
      const isConn = await sqliteServ.isConnection(dbNameRef.value, false);
  ...
  const setDopaCaseActive = async (dopaId: string) => {
  ...
  const getCommentById = async (commentId: string): Promise<HistoryComment[]> => {
